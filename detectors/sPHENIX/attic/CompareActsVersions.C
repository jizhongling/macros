...
void CompareActsVersions()
{
  gStyle->SetOptStat(0);
  TFile *df = TFile::Open("ActsDefaulttwentypion.root");
  //TFile *df = TFile::Open("Actsv17twentypionnewmmap.root");
  //TFile *nf = TFile::Open("Actsv17twentypion.root");

  //TFile *df = TFile::Open("highptplay.root");
  TFile *nf = TFile::Open("lowpttracksv17.root");

  //TFile *df = TFile::Open("../evalOutput/ActsHighOccDefault.root");
  //TFile *nf = TFile::Open("../evalOutput/ActsHighOccv17Test.root");
  ...
  ptcan->cd(1);
  dpt->Draw("colz");
  myText(0.1,0.03,kBlack,"Acts 1.1.0");
  ptcan->cd(2);
  npt->Draw("colz");
  myText(0.1,0.03,kBlack,"Acts 17.0.0");
  ...
  nptw->Draw("ep");
  dptw->Draw("epsame");
  myText(0.21,0.85,kBlack,"Acts 1.1.0");
  myText(0.21,0.77,kRed,"Acts 17.0.0");
  myText(0.21,0.69,kBlack,"40 pions");
  ...
  effleg->AddEntry(deff,"Acts v1.1.0 qual<10","P");
  effleg->AddEntry(neff,"Acts v17.0.0 qual<10","p");
  effleg->AddEntry(deffmaps,"Acts v1.1.0 nmaps>2","P");
  effleg->AddEntry(neffmaps,"Acts v17.0.0 nmaps>2","P");
  effleg->AddEntry((TObject*)0,"40 pions","");
  effleg->Draw("same");
