...
///
///     make
/// or  g++ -Wall `root-config --cflags --glibs` -lMLP -lMinuit -lTreePlayer -lTMVA -lTMVAGui -lXMLIO -lMLP -lm TMVAClassification.C -o TMVAClassification
...
int TMVAClassification( TString myMethodList = "" )
{
   ...
   TTree *signalTree     = ((TTree*)input->Get("ntp"))->CopyTree("abs(gflavor)==11 && TMath::Finite(ecemc+ehcalin+ehcalout)");
   ...
   dataloader->AddVariable( "fecemc := (ecemc)/(ecemc+ehcalin+ehcalout)", 'F' );
   dataloader->AddVariable( "fehcalin := (ehcalin)/(ecemc+ehcalin+ehcalout)", 'F' );
   dataloader->AddVariable( "fehcalout := (ehcalout)/(ecemc+ehcalin+ehcalout)", 'F' );
